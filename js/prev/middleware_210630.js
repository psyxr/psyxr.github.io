var prescriptionBtnState = 'closed';
var menuItems;

$(document).ready(function () {

    if (!($("#prescription_open_btn").length)) {
        prescriptionBtnState = 'opened';
        // console.log('Pres open btn does not exists')
    }
    ...
    initContentMenuScrollEvents(".rsb-content-links-active a");

    window.addEventListener("scroll", function() {
        // mobile css resolving
    ...
    $("#prescription_open_btn").click(function(e){
        ...
        resolvePrescriptionBtnStatus(menuItems);
        // Bind to scroll
    ...
    $(".right-sidebar-link").click(function(e) {
        // if (prescriptionBtnState === 'closed') {
    ...
    $("#concept").click(function(e, isScrolling = true) {

        $('.rsb-content').removeClass('rsb-content-links-active');
        $("#rsb-content-concept").addClass('rsb-content-links-active');

        $(".rsb-content").hide();
        $("#rsb-content-concept").show();

        $(".text-div").fadeOut();
        $("#concept-div").fadeIn();

        $(".rsb-btn").removeClass('rsb-btn-active');
        $(this).addClass('rsb-btn-active');
    ...
    $("#two-words").click(function(e, isScrolling = true) {

        $('.rsb-content').removeClass('rsb-content-links-active');
        $("#rsb-content-two-words").addClass('rsb-content-links-active');

        $(".rsb-content").hide();
        $("#rsb-content-two-words").show();

        $(".text-div").fadeOut();
        $("#two-words-div").fadeIn();
    ...
    $("#structural").click(function(e, isScrolling = true) {

        $('.rsb-content').removeClass('rsb-content-links-active');
        $("#rsb-content-structural").addClass('rsb-content-links-active');

        $(".rsb-content").hide();
        $("#rsb-content-structural").show();

        $(".text-div").fadeOut();
        $("#structural-div").fadeIn();
    ...
    // $("#ial-limitations").click(function(e) {
    //     $("#structural").trigger("click", [false]);
    //
    //     var anchorLink = $(this).attr("data-anchor-link");
    //
    //     var rsbLinks = $("#right-sidebar")
    //         .find(".rsb-content-links-active a");
    //
    //     var rsbLinksActive = rsbLinks.filter("[href='#"+jq(anchorLink)+"']");
    //
    //     rsbLinksActive.trigger("click");
    // });

    $(".internal-anchor-link").click(function(e) {
        let btnId = $(this).attr("data-btn-id");
        $("#"+btnId).click();

        let anchorLink = $(this).attr("data-anchor-link");

        console.log('anchorLink', anchorLink);

        if (anchorLink) {
            let rsbLinks = $("#right-sidebar").find(".rsb-content-links-active a");

            console.log('rsbLinks', rsbLinks);

            let rsbLinksActive = rsbLinks.filter("[href='#"+jq(anchorLink)+"']");

            console.log('rsbActive', rsbLinksActive);

            rsbLinksActive.click();
        }
    });
});
...
function cleanActiveRsbLink() {
    var rsbLinks = $("#right-sidebar")
        .find(".rsb-content-links-active a");

    // console.log("RSB_LINKS: ",rsbLinks);

    var rsbLinksActive = rsbLinks.filter(".rsl-active");

    // console.log("RSB_LINKS_ACTIVE: ", rsbLinksActive);

    rsbLinksActive.removeClass("rsl-active");

}

function initContentMenuScrollEvents(linkElement = "a") {

    var scrollingOffset = 80;

    // Cache selectors
    var rightMenu = $("#right-sidebar"),
        rightMenuHeight = 0, scrollItems;
        // All list items
        menuItems = rightMenu.find(linkElement);
        // Anchors corresponding to menu items
        scrollItems = menuItems.map(function(){
            let itemTmp = $(this).attr("href");
            // console.log("plain:",itemTmp);
            // console.log("jq:", jq(itemTmp));
            // console.log("needed:","#mindflow\\/interconnect\\?riding-a-motorcycle");

            // var item = $("#mindflow\\/interconnect\\?riding-a-motorcycle");
            var item = $(jq(itemTmp));
    ...
    menuItems.click(function(e){
        ...
        // console.log('prescrBtn status: ', prescriptionBtnState);
        ...
        var href = jq($(this).attr("href")),
            offsetTop = href === "#" ? 0 : $(href).offset().top - scrollingOffset;
    ...
    window.addEventListener("scroll", function() {
        ...
        var fromTop = $(this).scrollTop() + scrollingOffset + 30;
        ...
        // console.log("current scrolling item: ", cur);
        ...
        if (prescriptionBtnState === 'opened') {
            menuItems
                .removeClass("rsl-active");

            let filtered = menuItems.filter("[href='#"+jq(id)+"']");
            // console.log('filtered: ',filtered);

            let addedClass = filtered.addClass("rsl-active");
...
function resolvePrescriptionBtnStatus(menuItems) {

    $("#prescription_container").fadeToggle();
    console.log('prescrBtn state 1: ', prescriptionBtnState);
    if (prescriptionBtnState === 'closed') {
        prescriptionBtnState = 'opened';
    } else {
        prescriptionBtnState = 'closed';

        menuItems
            .removeClass("rsl-active");
    }
    console.log('prescrBtn state 2: ', prescriptionBtnState);
}
